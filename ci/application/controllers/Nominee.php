...
class Nominee extends CI_controller
{
    ...
    public function index()
    {
        if (!$this->session->userdata['logged_in']) {
            ...
            $teacher = $this->Nominee_model->all();
            $data = array();
        ...
        if (!$this->session->userdata('logged_in')) {
           ...
           $this->form_validation->set_rules('position', 'position', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->Nominee_model->create($this->table, $formArray);
    ...
    public function edit()
    {
        if (!$this->session->userdata['logged_in']) {
            ...
            $this->load->model('Nominee_model');
            $teacher = $this->Nominee_model->get($this->colid, $Id);
            $data = array();
            $data['teacher'] = $teacher;
            $this->form_validation->set_rules('n_id', 'n_id');
            $this->form_validation->set_rules('nominee', 'nominee', 'required');
            if ($this->form_validation->run() == false) {
                ...
                // $formArray['create_at']=date('Y-m-d');
                $this->Nominee_model->Update($table, $colid, $Id, $formArray);
                $this->session->set_flashdata('success', 'Record Added Successfully !');
                redirect(base_url() . 'index.php/Teacher/index');
    ...
    public function delete()
    {
        if (!$this->session->userdata['logged_in']) {
            ...
            $Id = $this->input->get('n_id');
            $this->load->model('Nominee_model');
            $teacher = $this->Nominee_model->get($colid, $Id);
            if (empty($teacher)) {
            ...
            $this->Nominee_model->delete($table, $colid, $Id);
